...
class _ScreenTimeStatsPageState extends State<ScreenTimeStatsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (remote.isNotEmpty) {
        final mapped = remote.map((m) => AppUsageStatModel(
  ...
  List<AppUsageStatModel> _filter(List<AppUsageStatModel> raw) {
    return raw
        .where((s) =>
            ...
            !_systemPrefixes.any((p) => s.packageName.startsWith(p)))
        .toList()
      ..sort((a, b) => b.totalMinutesToday.compareTo(a.totalMinutesToday));
  }

  int get _totalMinutes =>
      _stats.fold(0, (sum, s) => sum + s.totalMinutesToday);
  ...
  IconData _appIcon(String packageName) {
    if (packageName.contains('instagram') || packageName.contains('tiktok') ||
        packageName.contains('snapchat') || packageName.contains('twitter') ||
        packageName.contains('facebook') || packageName.contains('whatsapp') ||
    ...
    if (packageName.contains('chrome') || packageName.contains('browser') ||
        packageName.contains('firefox') || packageName.contains('safari')) {
      return Icons.language_rounded;
    }
    if (packageName.contains('teams') || packageName.contains('slack') ||
        packageName.contains('zoom') || packageName.contains('meet')) {
      return Icons.groups_rounded;
    }
    if (packageName.contains('gmail') || packageName.contains('mail') ||
    ...
    if (packageName.contains('camera') || packageName.contains('gallery') ||
      ...
      children: [
        ...
        ..._stats.asMap().entries.map((e) =>
            Padding(
              ...
              child: _buildAppRow(e.value, total, e.key),
            )),
        const SizedBox(height: 8),
        Text(
