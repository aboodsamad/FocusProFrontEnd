...
class _TOTState extends State<TrainOfThoughtPage> with TickerProviderStateMixin {
  // ── game data ──────────────────────────────────────────────────────────────
  int _level = 1;
  ...
  void initState() {
    super.initState();
    _winAnim = AnimationController(vsync: this, duration: const Duration(milliseconds: 700));
    _idleAnim = AnimationController(vsync: this, duration: const Duration(milliseconds: 2000))..repeat(reverse: true);
  ...
  void _onTick(Duration _) {
    ...
    final dt = _prevTime == null ? 0.016 : now.difference(_prevTime!).inMicroseconds / 1e6;
    _prevTime = now;
  ...
  void _onReached(TrainState train, NetworkNode node) {
    ...
    if (node.exitIds.isNotEmpty) {
      final nextId = node.isFork ? node.exitIds[node.activeExit] : node.exitIds[0];
  ...
  void _spawnTrain() {
    ...
    _trains.add(TrainState(uid: _uidCounter++, color: color, fromId: _cfg.tunnelId, toId: firstExit));
    _spawned++;
  }
            ...
            children: [
              ...
              Expanded(
                child: Stack(
                  children: [
                    _buildBoard(),
                    if (_phase == _Phase.idle) _IdleOverlay(level: _level, anim: _idleAnim, onStart: _startGame),
    ...
    return LayoutBuilder(
      builder: (ctx, box) {
        ...
        return GestureDetector(
          ...
          child: CustomPaint(
            size: Size(w, h),
            painter: _BoardPainter(nodes: _nodes, trains: _trains, stationFlash: _stationFlash, forkFlash: _forkFlash),
          ),
        );
      },
    );
...
class _BoardPainter extends CustomPainter {
  ...
  const _BoardPainter({required this.nodes, required this.trains, required this.stationFlash, required this.forkFlash});

  // ── helpers ────────────────────────────────────────────────────────────────

  Offset _pt(NetworkNode n, Size sz) => Offset(n.relX * sz.width, n.relY * sz.height);
  ...
  void _drawBackground(Canvas canvas, Size sz) {
    // base green field
    canvas.drawRect(Rect.fromLTWH(0, 0, sz.width, sz.height), Paint()..color = const Color(0xFF2E7D32));
  ...
  void _tree(Canvas canvas, double x, double y, Size sz) {
    ...
    for (final (scale, opacity) in [(1.0, 0.65), (0.7, 0.80)]) {
      final path = Path()
        ...
        ..lineTo(x - r * scale * 1.0, y + r * scale * 0.55)
        ..close();
      canvas.drawPath(path, Paint()..color = const Color(0xFF1B5E20).withOpacity(opacity));
    }
  }
  ...
  void _drawSegment(Canvas canvas, Size sz, NetworkNode a, NetworkNode b, {required bool active}) {
    final pa = _pt(a, sz);
    final pb = _pt(b, sz);
    ...
    canvas.drawLine(
      ...
      pb,
      Paint()
        ..color = active ? const Color(0xFF795548) : const Color(0xFF4E342E).withOpacity(0.45)
        ..strokeWidth = bedW
    ...
    if (active) {
      ...
      if (len > 0) {
        ...
        final railPaint = Paint()
          ...
          ..style = PaintingStyle.stroke;
        canvas.drawLine(pa + Offset(nx * off, ny * off), pb + Offset(nx * off, ny * off), railPaint);
        canvas.drawLine(pa - Offset(nx * off, ny * off), pb - Offset(nx * off, ny * off), railPaint);
  ...
  void _drawTunnel(Canvas canvas, Size sz) {
    final tunnel = nodes.values.firstWhere((n) => n.isTunnel, orElse: () => nodes.values.first);
    ...
    // tunnel mouth (arch)
    final archRect = Rect.fromCenter(center: Offset(p.dx, p.dy + r * 0.15), width: r * 1.0, height: r * 1.4);
    final archPath = Path()
      ..addArc(Rect.fromLTWH(archRect.left, archRect.top, archRect.width, archRect.width), pi, pi)
  ...
  void _drawForkNodes(Canvas canvas, Size sz) {
    for (final node in nodes.values) {
      ...
      // shadow
      canvas.drawCircle(p + const Offset(2, 3), r, Paint()..color = Colors.black.withOpacity(0.30));

      // body
      final bodyColor = Color.lerp(const Color(0xFF66BB6A), Colors.white, flash * 0.5)!;
  ...
  void _drawForkArrow(Canvas canvas, Size sz, Offset center, double r, NetworkNode node) {
    ...
    final aPath = Path()
      ...
      ..lineTo((tip - Offset(ux * ah, uy * ah) + perp * aw).dx, (tip - Offset(ux * ah, uy * ah) + perp * aw).dy)
      ..lineTo((tip - Offset(ux * ah, uy * ah) - perp * aw).dx, (tip - Offset(ux * ah, uy * ah) - perp * aw).dy)
      ..close();
  ...
  void _drawBuilding(Canvas canvas, Size sz, Offset center, TrainColor color, double flash) {
    ...
    // shadow
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTRB(left + 2, bodyTop + 3, right + 2, bottom + 3), const Radius.circular(3)),
      Paint()..color = Colors.black.withOpacity(0.28),
    );

    // body
    final bodyRect = RRect.fromRectAndRadius(Rect.fromLTRB(left, bodyTop, right, bottom), const Radius.circular(3));
    canvas.drawRRect(bodyRect, Paint()..color = ac);

    // body highlight
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTRB(left, bodyTop, left + bw * 0.22, bottom), const Radius.circular(3)),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTRB(center.dx - doorW / 2, bottom - doorH, center.dx + doorW / 2, bottom),
    ...
    for (final xo in [-bw * 0.26, bw * 0.26]) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTRB(center.dx + xo - winW / 2, winY, center.dx + xo + winW / 2, winY + winH),
          const Radius.circular(2),
        ),
    ...
    // star on rooftop
    _drawStar(canvas, center.dx, roofTop - sz.height * 0.012, sz.width * 0.018, ac);
  }
  ...
  void _drawOneTrain(Canvas canvas, Size sz, TrainState train) {
    ...
    final bodyRR = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(-r * 0.08, 0), width: cw, height: ch),
    ...
    // highlight
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTRB(-cw * 0.42, -ch * 0.45, cw * 0.30, -ch * 0.05), Radius.circular(ch * 0.30)),
    ...
    // window
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTRB(cw * 0.04, -ch * 0.37, cw * 0.38, ch * 0.30), Radius.circular(ch * 0.12)),
...
class _Header extends StatelessWidget {
  ...
  const _Header({required this.level, required this.correct, required this.total, required this.onBack});
        ...
        children: [
          ...
          Row(
            children: [
              ...
              _StatPill(label: 'CORRECT', value: '$correct/$total', color: AppColors.secondaryContainer),
              const SizedBox(width: 8),
              _StatPill(label: 'ACCURACY', value: '$accuracy%', color: AppColors.primaryFixed),
            ],
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            label,
            style: TextStyle(color: color.withOpacity(0.7), fontSize: 8, fontWeight: FontWeight.w700, letterSpacing: 1),
          ),
          Text(
            value,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            label,
            style: TextStyle(color: Colors.grey[500], fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 1.2),
          ),
...
class _IdleOverlay extends StatelessWidget {
  ...
  const _IdleOverlay({required this.level, required this.anim, required this.onStart});
        ...
        child: Center(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: const Color(0xFF8B5CF6).withOpacity(0.2), blurRadius: 40, spreadRadius: 4)],
            ),
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                AnimatedBuilder(
                  animation: anim,
                  builder: (_, child) => Transform.translate(offset: Offset(0, -6 * anim.value), child: child),
                  child: Container(
                    width: 74,
                    height: 74,
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(color: const Color(0xFF8B5CF6).withOpacity(0.5), blurRadius: 24, spreadRadius: 4),
                      ],
                    ),
                    child: const Icon(Icons.train_rounded, color: Colors.white, size: 38),
                  ),
                ),
                ...
                Text(
                  'Route trains to matching color stations',
                  style: TextStyle(color: Colors.grey[400], fontSize: 13),
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 16),
                const _Hint(icon: Icons.touch_app_rounded, text: 'Tap the circles to switch tracks'),
                const SizedBox(height: 6),
                const _Hint(icon: Icons.compare_arrows_rounded, text: 'Route each train to its matching station'),
                const SizedBox(height: 6),
                const _Hint(icon: Icons.directions_railway_rounded, text: 'Trains leave one by one'),
                const SizedBox(height: 24),
                _GreenBtn(label: level == 1 ? 'Start Game' : 'Level $level', onTap: onStart),
              ],
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 8),
        Flexible(
          child: Text(text, style: TextStyle(color: Colors.grey[400], fontSize: 12.5)),
        ),
      ],
    );
  }
        ...
        child: Center(
          child: AnimatedBuilder(
            ...
            builder: (_, child) => Transform.scale(
              scale: 0.85 + 0.15 * CurvedAnimation(parent: anim, curve: Curves.elasticOut).value,
              child: child,
            ),
            child: Container(
              margin: const EdgeInsets.symmetric(horizontal: 28),
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: Colors.white.withOpacity(0.08)),
                boxShadow: [
                  BoxShadow(color: const Color(0xFF34D399).withOpacity(0.2), blurRadius: 40, spreadRadius: 4),
                ],
              ),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(color: const Color(0xFF34D399).withOpacity(0.45), blurRadius: 24, spreadRadius: 4),
                      ],
                    ),
                    ...
                    children: List.generate(3, (i) {
                      ...
                      return Padding(
                        ...
                        child: Icon(
                          ...
                          color: lit ? const Color(0xFFFBBF24) : Colors.grey[700],
                          size: 36,
                        ),
                      );
                    }),
                  ...
                  if (wrong > 0) ...[const SizedBox(height: 6), _Row(label: 'Wrong station', value: '$wrong')],
                  const SizedBox(height: 24),
          ...
          child: AnimatedBuilder(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: Colors.white.withOpacity(0.08)),
                boxShadow: [
                  BoxShadow(color: const Color(0xFFEF4444).withOpacity(0.2), blurRadius: 40, spreadRadius: 4),
                ],
              ),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(color: const Color(0xFFEF4444).withOpacity(0.45), blurRadius: 24, spreadRadius: 4),
                      ],
                    ),
                    child: const Icon(Icons.train_rounded, color: Colors.white, size: 36),
                  ),
                  const SizedBox(height: 14),
                  const Text(
                    'Too Many Crashes!',
                  ...
                  const SizedBox(height: 6),
                  Text('Level $level', style: TextStyle(color: Colors.grey[400], fontSize: 14)),
                  const SizedBox(height: 16),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(color: const Color(0xFF8B5CF6).withOpacity(0.45), blurRadius: 16, offset: const Offset(0, 6)),
          ],
        ),
        child: Text(
          label,
