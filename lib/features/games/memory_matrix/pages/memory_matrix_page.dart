...
class _MemoryMatrixPageState extends State<MemoryMatrixPage> with TickerProviderStateMixin {
  ...
  void _startGame() {
    ...
    setState(() {
      _game = MemoryMatrixState.initial(gs).copyWith(level: widget.startLevel, phase: MemoryMatrixPhase.countdown);
    });
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) async {
        if (didPop) return;
        final isPlaying =
            _game.phase != MemoryMatrixPhase.idle &&
      ...
      child: ScaleTransition(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('Level ${_game.level - 1} cleared', style: TextStyle(color: _textMuted, fontSize: 16)),
        ...
        child: Column(
          ...
          children: [
            Container(
              width: 88,
              height: 88,
              decoration: BoxDecoration(shape: BoxShape.circle, color: _wrong.withOpacity(0.15)),
            ...
            const SizedBox(height: 6),
            Text('Level ${widget.startLevel} incomplete', style: TextStyle(color: _textMuted, fontSize: 14)),
            const SizedBox(height: 28),
            MemoryMatrixStatRow(label: 'Score', value: '${_game.score} pts', valueColor: _accent),
            const SizedBox(height: 8),
