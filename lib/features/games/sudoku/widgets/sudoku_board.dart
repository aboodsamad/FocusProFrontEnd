...
class SudokuBoard extends StatelessWidget {
  ...
  Color _cellBg(int row, int col) {
    ...
    final selVal = (selectedRow != null && selectedCol != null) ? board[selectedRow!][selectedCol!] : 0;
    final isSelected = selectedRow == row && selectedCol == col;
    final isInCross = !isSelected && (selectedRow == row || selectedCol == col);
    final isInBox =
        !isSelected &&
        !isInCross &&
        selectedRow != null &&
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(0.04), blurRadius: 40, spreadRadius: 2)],
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(24),
        child: LayoutBuilder(
          builder: (ctx, constraints) {
            ...
            return Column(
              children: List.generate(9, (row) => Row(children: List.generate(9, (col) => _buildCell(row, col, size)))),
            );
          },
        ),
      ),
