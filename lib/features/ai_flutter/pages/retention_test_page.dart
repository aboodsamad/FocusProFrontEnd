...
class _RetentionTestPageState extends State<RetentionTestPage> with SingleTickerProviderStateMixin {
  // ── State ─────────────────────────────────────────────────────────────────
      ...
      child: Column(
        ...
        children: [
          Container(
            width: 80,
            height: 80,
            decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.primaryContainer),
            child: const Center(child: CircularProgressIndicator(color: AppColors.primaryFixed, strokeWidth: 2.5)),
          ),
          const SizedBox(height: 28),
          Text(
            'AI is building your retention test…',
            style: TextStyle(color: AppColors.onSurface, fontSize: 15, fontWeight: FontWeight.w500),
          ),
          const SizedBox(height: 8),
          Text('Pulling from your past reading', style: TextStyle(color: AppColors.onSurfaceVariant, fontSize: 13)),
        ],
      ),
    );
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(color: AppColors.onSurfaceVariant, fontSize: 15, height: 1.6),
            ),
        ...
        if (chosen != null)
          Container(
            ...
            child: _PrimaryButton(
              label: isLast ? (_submitting ? 'Submitting…' : 'Submit Test') : 'Next Question',
  ...
  Widget _buildResult() {
    ...
    final pct = r.totalQuestions > 0 ? (r.correctCount / r.totalQuestions * 100).round() : 0;
                    ...
                    children: [
                      ...
                      Text(
                        gained
                            ...
                            ? 'Needs Review'
                            : 'Decent Retention',
                        style: const TextStyle(color: AppColors.onSurface, fontSize: 20, fontWeight: FontWeight.w700),
                      ),
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text(
                                        gained
                                            ? '+${r.scoreDelta.toStringAsFixed(1)}'
                                            : r.scoreDelta.toStringAsFixed(1),
                                        style: TextStyle(
                                          color: AppColors.primary,
                                          fontSize: 16,
...
class _OptionTile extends StatelessWidget {
  ...
  const _OptionTile({required this.letter, required this.text, required this.isSelected, required this.onTap});
      ...
      child: Column(
        ...
        children: [
          ...
          if (!correct) ...[
            const SizedBox(height: 8),
            Text(
              'Your answer:    ${result.chosenAnswer}  ${question.optionText(result.chosenAnswer)}',
              style: TextStyle(color: AppColors.error, fontSize: 12),
            ),
            Text(
              'Correct answer: ${result.correctAnswer}  ${question.optionText(result.correctAnswer)}',
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              if (trailing != null) ...[const SizedBox(width: 8), Icon(trailing, color: AppColors.onPrimary, size: 20)],
            ],
          ),
        ),
      ),
